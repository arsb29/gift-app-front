...
export function RecentAction(props: Props) {
  ...
  switch (action.type) {
    case ACTION_TYPE.buy:
      description = <div>-{gift.amount} {gift.asset}</div>;
      break;
    case ACTION_TYPE.send:
      if (receiver) description = <div>to <span>{formatName(receiver)}</span></div>;
      break;
    case ACTION_TYPE.receive:
      description = <div>from <span>{formatName(sender)}</span></div>;
      break;
    default:
      break;
  }
    ...
    <div className={cc(styles.container)}>
      <div className={styles.image}>
        ...
        {actionTypeIcon && actionTypeIconClassName && (
          <IconSmallActionType actionTypeIcon={actionTypeIcon} className={cc(styles.actionTypeIcon, actionTypeIconClassName)} />
        )}
      </div>
